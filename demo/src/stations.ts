...
/**
 * Showcases: whole pages rather than mechanisms.
 *
 * A station isolates one capability so you can feel it. That is necessary and it is not
 * sufficient — a framework can win every isolated station and still be miserable to build a page
 * with. So these are real pages, each one leaning on several capabilities at once, and each one
 * named for the shape of application it stands in for.
 *
 * The interesting number on a showcase is not a microbenchmark. It is what the page cost: bytes
 * on the wire, first byte, how much JavaScript ran, and how many of its regions are shared cache
 * entries rather than per-user renders.
 */
export interface Showcase {
  id: string
  title: string
  /** The shape of application this stands in for. */
  standsFor: string
  /** What makes it hard, and which capabilities carry it. */
...
export const SHOWCASES: readonly Showcase[] = [
  {
    ...
    leansOn: [
      ...
      'Props passed down and projected into the child’s holes, with no per-instance mount',
      'A route param, which becomes a cache key component without the plan mentioning keys',
      'Every region buffered, so the plan lowers to in-order and the ~1 KB out-of-order filler is never on the wire',
      'The point being that the streaming machinery is opt-in per slot rather than the price of using the framework',
    ],
    status: 'live',
  },
  {
    ...
    standsFor:
      'A news front page, a product listing, a search result — hundreds of rows, mostly shared, changing under you',
    leansOn: [
      'One sealed row template projected hundreds of times, so page weight tracks content and not components',
      'Out-of-order streaming, so the slow region does not hold the first byte',
      'A shared cache class, because nothing on it reads identity',
      'Incremental recompute: the tick that changes six rows costs six row renders',
  ...
  },
  {
    ...
    standsFor: 'Anything with a signed-in user inside an otherwise shareable page',
    leansOn: [
      'Contagion: one private fragment inside a shared shell, isolated into its own cache unit rather than making the page private',
      'Intents: the only thing allowed to write, declaring the tags it invalidates',
      'An optimistic epoch whose rollback is a discarded epoch rather than reconstructed state',
      'A form that works with JavaScript off, answered with a 303',
      'Push invalidation reaching the other tab that was holding the same key',
    ],
    status: 'live',
  },
  {
    ...
    standsFor: 'An admin page, an analytics view — several independent queries of very different cost',
    leansOn: [
      'A wave schedule derived from data dependencies, not from declaration order',
      'Per-slot CPU budgets, and five different pages one breach can produce',
      'A worker pool, where a budget is a limit rather than a report',
      'Per-slot cache policies, so the cheap panels are shared and the expensive one is not',
      'Early hints, so discovery is not downstream of the slowest query',
    ],
    status: 'live',
  },
  {
    id: 'article',
    title: 'An article',
    standsFor: 'Documentation, a blog post, a marketing page — the case where almost nothing should ship',
    leansOn: [
      'A content route: adopt and bind, no update path, no persistence',
      'A fragment that reads nothing, so its class is static and its key is its content',
      'Escape elision, kept for correctness rather than for throughput',
      'The floor: it renders with no JavaScript at all, and the runtime only adds what is interactive',
    ],
    status: 'live',
  },
]
