...
import { chainFor, createApp, discover, type DiscoveredNested } from 'weft/server'
...
const OUTER = (holes: string) => `import { fragment, raw } from 'weft'
...
const INNER = (holes: string) => `import { fragment } from 'weft'
...
const PAGE = (slots: string[]) => `import { defineRoute } from 'weft'
...
test('a hole in a nested layout that nothing supplies fails the build with that file named', async () => {
  const root = await fixture('unfilled-value', {
    'app/layout.tsx': OUTER('panel body'),
    'app/routes/section/index.data.ts': PAGE(['panel', 'toc', 'body']),
    'app/routes/section/layout.tsx': `import { fragment } from 'weft'
