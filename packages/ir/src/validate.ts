...
export function validateTemplate(ir: TemplateIR): ValidationResult {
  ...
  ir.holes.forEach((h, i) => {
    ...
    if (h.nested !== undefined) {
      if (h.kind !== 'list' && h.kind !== 'component' && h.kind !== 'variant')
        fail(
          'E_NESTED_KIND',
          `holes[${i}]`,
          'only a list, component or variant hole can name a nested template',
        )
  ...
  for (const f of ir.forms) {
    // `remote`'s equivalence cannot be proven here: the bytes are another deployment's. See `spec/kernel/composition.md`.
