...
The long form. [`README.md`](README.md) is the introduction and carries the headline numbers; this
file is what used to sit under it — the mechanism, the byte budgets, what measurement changed, and
every refusal with its argument. The per-capability reference is [`spec/`](spec/), the live version
is `packages/inspector`, and the claim-by-claim measurement ledger is
...
| Front door — the code, bundled            | **14,385** | 15,919 | 46,599 | 15,360  |
| Server kernel — the document request path | **8,273**  | 9,288  | 24,241 | 8,320   |
| Kernel + intent dispatch                  | **9,802**  | 11,049 | 29,321 | 10,240  |
| Kernel + surgical refresh and epochs      | **11,075** | 12,388 | 33,359 | 12,288  |
| Kernel + the patch encoder                | **11,754** | 13,128 | 35,830 | 12,288  |
| Kernel + authority                        | **11,722** | 13,183 | 35,428 | 12,288  |
| Kernel + composition                      | **11,517** | 12,953 | 34,676 | 12,288  |
| Kernel + a live Warp channel              | **14,016** | 15,635 | 42,326 | 14,336  |
| Kernel + composition over a live channel  | **17,119** | 19,115 | 52,345 | 17,408  |
...
written — **25,835 B brotli** for the demo, agreeing within 0.34% with the same walk over HTTP. That is now the gated number: `budget({ js, grow })` in the plan is enforced by
