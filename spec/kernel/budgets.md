...
| `entry-channel.ts`        | The above, plus surgical refresh, form selection, epochs, the stale registry         | 11,075 B | 12,288 B | No design figure. A watermark                                           |
| `entry-patch.ts`          | The above, plus the patch encoder: the surgical rung a template needs no proof for   | 11,754 B | 12,288 B | No design figure. Its own, because in the refresh path it cost everyone |
...
| `entry-transport.ts`      | The channel path, plus a live channel: negotiation, held state, push invalidation    | 14,016 B | 14,336 B | No design figure. A watermark, **moved from 13,312**                    |
| `entry-stage.ts`          | The above, plus a whole route staged over the channel: `WARM at=`, `NAV`             | 14,413 B | 15,360 B | No design figure. Its own, because it went past the watermark above     |
| `entry-discover.ts`       | The above, plus lazy plan extension: `WARM plan=`, `PLAN`                            | 14,578 B | 15,360 B | No design figure. Its own, on the rule route staging established        |
| `entry-render.ts`         | The transport, plus a catalogue of fragments a client can name: render intents       | 14,576 B | 14,592 B | No design figure. Its own, on the same rule                             |
| `entry-region.ts`         | The request path, plus regions resolved through the registry and checked on arrival  | 11,517 B | 12,288 B | No design figure. Its own, on the same rule. **Moved from 11,264**      |
| `entry-region-channel.ts` | The transport plus composition: a region refreshed over a live channel               | 17,119 B | 17,408 B | No design figure. Its own, and **moved from 16,384**                    |
...
| `boot.ts` (front door) | The front door's code, bundled and minified — see below | 14,385 B | 15,360 B |
...
**The front-door watermark moved to 15 KB, for discovery that reaches an ordinary page.** 12 KB when
the exposed table landed, 13 KB when the refresh interval and the patch applier did, 14 KB when the
channel got a WebSocket with a fallback to the two fetches, and 15 KB when the plan started arriving
on pages that never opened a channel. The last one bought four things a page had been paying for and
not receiving: an opening turn on the half-duplex binding, so the frames a server sends unasked
arrive on a deployment that cannot hold a connection; a re-declaration after a navigation, so the
plan is about the page the reader is on; a history traversal that stages instead of reloading; and a
viewport rule whose ceiling follows the staging table rather than counting up once and stopping. It
is a watermark with no design figure behind it and it moves with a reason written down; what it is
_not_ is the number a reader pays, which is the paragraph below.
